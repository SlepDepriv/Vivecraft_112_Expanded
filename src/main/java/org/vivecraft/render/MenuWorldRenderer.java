...
public class MenuWorldRenderer {
	...
	public void init() {
		if (((IMinecraftVR)mc).getVRSettings().menuWorldSelection == VRSettings.MENU_WORLD_NONE) {
		...
		try {
			...
			if (inputStream != null) {
				...
				prepare();
				((IEntityRendererVR)mc.entityRenderer).setMenuWorldFastTime(new Random().nextInt(10) == 0);
			} else {
...
//                if (Config.isCustomColors())
    ...
    public void renderClouds(int pass, double x, double y, double z)
    {
        if (this.worldProvider.isSurfaceWorld()) {
	        ...
	        float f = 12.0F;
	        float f1 = 4.0F;
	        double d0 = ((IMinecraftVR)mc).getTickCounter() + mc.getRenderPartialTicks();
	        ...
	        f2 = f2 + mc.gameSettings.clouds * 128.0F;
    ...
    private void generateSky() throws Exception
    {
        ...
        ((IRenderGlobalVR)mc.renderGlobal).triggerRenderSky(vertexbuffer, 16.0F, false);
    ...
    private void generateSky2() throws Exception
    {
        ...
        ((IRenderGlobalVR)mc.renderGlobal).triggerRenderSky(vertexbuffer, -16.0F, true);
        vertexbuffer.finishDrawing();
        vertexbuffer.reset();
    ...
    private void generateStars() throws Exception
    {
        ...
        ((IRenderGlobalVR)mc.renderGlobal).triggerRenderStars(vertexbuffer);
        vertexbuffer.finishDrawing();
        vertexbuffer.reset();
    ...
    public static class MenuCloudRenderer
    {
        ...
        public boolean shouldUpdateGlList(float posY)
        {
            ...
            else if (((IMinecraftVR)mc).getTickCounter() >= this.cloudTickCounterUpdate + 100)
            {
                return true;
            }
            else if (!this.color.equals(this.lastColor) && ((IMinecraftVR)mc).getTickCounter() >= this.cloudTickCounterUpdate + 1)
            {
                return true;
            }
            else
            {
                boolean flag = this.cloudPlayerY < 128.0D + (double)(this.mc.gameSettings.clouds * 128.0F);
                boolean flag1 = posY < 128.0D + (double)(this.mc.gameSettings.clouds * 128.0F);
                return flag1 != flag;
            }
        }
        ...
        {
            GL11.glEndList();
            this.cloudTickCounterUpdate = ((IMinecraftVR)mc).getTickCounter();
            this.cloudPlayerX = x;
            this.cloudPlayerY = y;
            this.cloudPlayerZ = z;
            this.updated = true;
        ...
        {
            double d3 = (((IMinecraftVR)mc).getTickCounter() - this.cloudTickCounterUpdate) + this.partialTicks;
