...
public class SwingTracker extends Tracker{
	...
	public SwingTracker(IMinecraftVR mc) {
		super(mc);
	}
	...
	public boolean isActive(EntityPlayerSP p){
		...
		if (mc.getVRSettings().weaponCollision == 0)
			return false;
		if (mc.getVRSettings().weaponCollision == 2)
			return !p.isCreative();
		if (mc.getVRSettings().seated)
			return false;
		if(mc.getVRSettings().vrFreeMoveMode == mc.getVRSettings().FREEMOVE_RUNINPLACE && p.moveForward > 0){
			return false; //dont hit things while RIPing.
		}
		if(p.isActiveItemStackBlocking()){
			return false; //dont hit things while blocking.
		}
		if(mc.getJumpTracker().isjumping())
			return false;
		return true;    
	}
	...
	public void doProcess(EntityPlayerSP player){ //on tick
        ...
        ((Minecraft)mc).profiler.startSection("updateSwingAttack");
        
        
        for(int c =0 ;c<2;c++){

        	if (mc.getClimbTracker().isGrabbingLadder(c)) continue;
        	
        	Vec3d handPos = mc.getVRPlayer().vrdata_world_pre.getController(c).getPosition();
        	Vec3d handDirection = mc.getVRPlayer().vrdata_world_pre.getHand(c).getCustomVector(forward);
        	...
        	weaponLength *= mc.getVRPlayer().vrdata_world_pre.worldScale;
			...
			tipHistory[c].add(weaponEnd[c].subtract(mc.getVRPlayer().vrdata_world_pre.origin));
...
//        	int passes = (int) (tickDist / .1f); //TODO someday....
        	...
        	List entities = ((Minecraft)mc).world.getEntitiesWithinAABBExcludingEntity(
					((Minecraft)mc).player, weaponBB);
        	for (int e = 0; e < entities.size(); ++e)
        	{
        		Entity hitEntity = (Entity) entities.get(e);
        		if (hitEntity.canBeCollidedWith() && !(hitEntity == ((Minecraft)mc).player.getRidingEntity()) )
        		{
        			if(mc.getVRSettings().animaltouching && hitEntity instanceof EntityAnimal && !tool && !lastWeaponSolid[c] && !player.isInWater()){
						((Minecraft)mc).playerController.interactWithEntity(player, hitEntity, c==0?EnumHand.MAIN_HAND:EnumHand.OFF_HAND);
        				disableSwing = 3;
        			...
        			{
        				if(canact){
							((Minecraft)mc).playerController.attackEntity(player, hitEntity);
        	...
        	if(!inAnEntity && !sword){
        		...
        		IBlockState block = ((Minecraft)mc).world.getBlockState(bp);
        		...
        		RayTraceResult col = ((Minecraft)mc).world.rayTraceBlocks(lastWeaponEndAir[c], weaponEnd[c], true, false, true);

        		if (col != null) ((IPlayerControllerMPVR)((Minecraft)mc).playerController).setHitVecOverride(col.hitVec);
        		...
        		if (flag && (shouldIlookatMyHand[c] || (col != null && col.typeOfHit == Type.BLOCK)))
        		{
        					...
        					if(item == Items.BUCKET) {       						
        						...
        						if (IAmLookingAtMyHand[c]){
        							...
        							if(	Minecraft.getMinecraft().playerController.processRightClick(player, player.world,c==0?EnumHand.MAIN_HAND:EnumHand.OFF_HAND)==EnumActionResult.SUCCESS){
										Minecraft.getMinecraft().entityRenderer.itemRenderer.resetEquippedProgress(c==0?EnumHand.MAIN_HAND:EnumHand.OFF_HAND);
        							}
        						}
        					}
        				} else {
        					if(canact && (!mc.getVRSettings().realisticClimbEnabled || (!(block.getBlock() instanceof BlockLadder) && !(block.getBlock() instanceof BlockVine)))) {
        						int p = 3;
        						if(item instanceof ItemHoe){
									...
									((Minecraft)mc).playerController.clickBlock(col.getBlockPos(), col.sideHit);
        							if(getIsHittingBlock()) { //seems to be the only way to tell it didnt insta-broke.
        								...
        								{	//send multiple ticks worth of 'holding left click' to it.
        									...
        									if(((Minecraft)mc).playerController.onPlayerDamageBlock(col.getBlockPos(), col.sideHit))
												Minecraft.getMinecraft().effectRenderer.addBlockHitEffects(col.getBlockPos(), col.sideHit);
        							...
        							mc.getVRPlayer().blockDust(col.hitVec.x, col.hitVec.y, col.hitVec.z, 3*p, block);
        							lastWeaponSolid[c] = true;
        						}
        			...
        			((IPlayerControllerMPVR)((Minecraft)mc).playerController).setHitVecOverride(null);
        		}
        	}
