...
public class EatingTracker extends Tracker{
	...
	public EatingTracker(IMinecraftVR mc) {
		super(mc);
	}
...
private Random r = new Random();
	...
	public void doProcess(EntityPlayerSP player){
		...
		for(int c=0;c<2;c++){
			...
			controllerPos = controllerPos.add(mc.getVRPlayer().vrdata_room_pre.getController(c).getDirection().scale(0.1));
			...
			if(mouthPos.distanceTo(controllerPos)<threshold){
				...
				if(!eating[c]){
					if(	((Minecraft)mc).playerController.processRightClick(player, player.world,c==0?EnumHand.MAIN_HAND:EnumHand.OFF_HAND)==EnumActionResult.SUCCESS){
						((Minecraft)mc).entityRenderer.itemRenderer.resetEquippedProgress(c==0?EnumHand.MAIN_HAND:EnumHand.OFF_HAND);
						eating[c]=true;
