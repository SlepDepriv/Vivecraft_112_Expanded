...
public class InteractTracker extends Tracker{
	...
	public InteractTracker(IMinecraftVR mc) {
		super(mc);
	}

	public boolean isActive(EntityPlayerSP p){
		if(((Minecraft)mc).playerController == null) return false;
	...
	public void processBindings() {
		for(int c =0 ;c<2;c++){
			if(MCOpenVR.keyVRInteract.isPressed(ControllerType.values()[c])) {
				...
				if(hotbar >= 0 && hotbar < 9 && ((Minecraft)mc).player.inventory.currentItem != hotbar && hand == EnumHand.MAIN_HAND) {
					((Minecraft)mc).player.inventory.currentItem = hotbar;
					success = true;
				}
				else if(hotbar == 9 && hand == EnumHand.MAIN_HAND) {
					((Minecraft)mc).player.connection.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.SWAP_HELD_ITEMS, BlockPos.ORIGIN, EnumFacing.DOWN));
					success = true;
				}
