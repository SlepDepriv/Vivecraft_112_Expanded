...
public class GuiHandler {
	...
	public static VRFrameBuffer guiFramebuffer = null;
	...
	public static void processGui() {
		...
		Vec2f tex = getTexCoordsForCursor(guiPos_room, guiRotation_room, mc.currentScreen, guiScale, ((IMinecraftVR)mc).getVRPlayer().vrdata_room_pre.getController(0));
	...
	public static void processBindingsGui() {
		...
		{ //press left mouse button
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).mouseDown(mouseX, mouseY, 0, true);
			lastPressedLeftClick = true;
		}

		if (keyLeftClick.isKeyDown() && mc.currentScreen != null)
			((IGuiScreenVR)mc.currentScreen).mouseDrag(mouseX, mouseY);//Signals mouse move
		...
		if (!keyLeftClick.isKeyDown() && lastPressedLeftClick) {
			...
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).mouseUp(mouseX, mouseY, 0, true);
		...
		if (keyRightClick.isPressed() && mc.currentScreen != null && mouseValid) {
			...
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).mouseDown(mouseX, mouseY, 1, true);
		...
		if (!keyRightClick.isKeyDown() && lastPressedRightClick) {
			...
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).mouseUp(mouseX, mouseY, 1, true);
		...
		if (keyMiddleClick.isPressed() && mc.currentScreen != null && mouseValid) {
			...
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).mouseDown(mouseX, mouseY, 2, true);
		...
		if (!keyMiddleClick.isKeyDown() && lastPressedMiddleClick) {
			...
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).mouseUp(mouseX, mouseY, 2, true);
		...
		if (keyShift.isPressed() && mc.currentScreen != null)
		{
			...
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).setPressShiftFake(true);
		...
		if (!keyShift.isKeyDown() && lastPressedShift)
		{
			...
			if (Display.isActive())
				...
				((IGuiScreenVR)mc.currentScreen).setPressShiftFake(false);
	...
	public static void onGuiScreenChanged(GuiScreen previousScreen, GuiScreen newScreen, boolean unpressKeys)
	{
		...
		if (mc.world == null || newScreen instanceof GuiWinGame) {
			((IMinecraftVR)mc).getVRSettings().vrWorldRotationCached = ((IMinecraftVR)mc).getVRSettings().vrWorldRotation;
			((IMinecraftVR)mc).getVRSettings().vrWorldRotation = 0;
		} else { //these dont update when screen open.
			if (((IMinecraftVR)mc).getVRSettings().vrWorldRotationCached != 0) {
				((IMinecraftVR)mc).getVRSettings().vrWorldRotation = ((IMinecraftVR)mc).getVRSettings().vrWorldRotationCached;
				((IMinecraftVR)mc).getVRSettings().vrWorldRotationCached = 0;
			}
		}

		boolean staticScreen = mc.entityRenderer == null || ((IEntityRendererVR)mc.entityRenderer).isInMenuRoom();
		staticScreen &= !((IMinecraftVR)mc).getVRSettings().seated && !((IMinecraftVR)mc).getVRSettings().menuAlwaysFollowFace;
		...
		if((previousScreen==null && newScreen != null) || (newScreen instanceof GuiChat || newScreen instanceof GuiScreenBook || newScreen instanceof GuiEditSign))
		{
			...
			if(appearOverBlock && mc.objectMouseOver != null && mc.objectMouseOver.typeOfHit == RayTraceResult.Type.BLOCK){
				...
				Vec3d temp_room = ((IMinecraftVR)mc).getVRPlayer().world_to_room_pos(temp, ((IMinecraftVR)mc).getVRPlayer().vrdata_world_pre);
				Vec3d pos = ((IMinecraftVR)mc).getVRPlayer().vrdata_room_pre.hmd.getPosition();
				...
				guiPos_room = ((IMinecraftVR)mc).getVRPlayer().world_to_room_pos(guiPosWorld, ((IMinecraftVR)mc).getVRPlayer().vrdata_world_pre);
				...
				Vec3d v = ((IMinecraftVR)mc).getVRPlayer().vrdata_room_pre.hmd.getPosition();
				Vec3d e = ((IMinecraftVR)mc).getVRPlayer().vrdata_room_pre.hmd.getCustomVector(adj);
				guiPos_room = new Vec3d(
						(e.x  / 2 + v.x),
						(e.y / 2 + v.y),
						(e.z / 2 + v.z));
	...
	public static Vec3d applyGUIModelView(RenderPass currentPass)
	{
		...
		System.out.println("GUIPOS at start iS:");
		System.out.println(guipos);

		if(guipos == null){
			System.out.println("GUIPOS is null!");
			guirot = null;
			scale = 1;
			if (mc.world!=null && (mc.currentScreen==null || ((IMinecraftVR)mc).getVRSettings().floatInventory == false))
			{  // HUD view - attach to head or controller
				int i = 1;
				if (((IMinecraftVR)mc).getVRSettings().vrReverseHands) i = -1;
				...
				//					}
				if (((IMinecraftVR)mc).getVRSettings().seated || ((IMinecraftVR)mc).getVRSettings().vrHudLockMode == VRSettings.HUD_LOCK_HEAD)
				{
					Matrix4f rot = Matrix4f.rotationY((float)((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.rotation_radians);
					Matrix4f max = Matrix4f.multiply(rot, MCOpenVR.hmdRotation);

					Vec3d v = ((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.hmd.getPosition();
					Vec3d d = ((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.hmd.getDirection();

					if(((IMinecraftVR)mc).getVRSettings().seated && ((IMinecraftVR)mc).getVRSettings().seatedHudAltMode){
						d = ((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.getController(0).getDirection();
						max = Matrix4f.multiply(rot, MCOpenVR.getAimRotation(0));
					}

					guipos = new Vec3d((v.x + d.x*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale*((IMinecraftVR)mc).getVRSettings().hudDistance),
							(v.y + d.y*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale*((IMinecraftVR)mc).getVRSettings().hudDistance),
							(v.z + d.z*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale*((IMinecraftVR)mc).getVRSettings().hudDistance));
					System.out.println("GUIPOS at 1 iS:");
					System.out.println(guipos);
					...
					scale = ((IMinecraftVR)mc).getVRSettings().hudScale;

				}else if (((IMinecraftVR)mc).getVRSettings().vrHudLockMode == VRSettings.HUD_LOCK_HAND)//hud on hand
				{
					...
					guiLocal = new Vec3d(guiLocal.x, 0.32*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale,guiLocal.z);

					guipos = ((IEntityRendererVR)mc.entityRenderer).getControllerRenderPos(1);
					System.out.println("GUIPOS at 2 iS:");
					System.out.println(guipos);

					MCOpenVR.hudPopup = true;

				}
				else if (((IMinecraftVR)mc).getVRSettings().vrHudLockMode == VRSettings.HUD_LOCK_WRIST)//hud on wrist
				{
					...
					guipos = ((IEntityRendererVR)mc.entityRenderer).getControllerRenderPos(1);
					System.out.println("GUIPOS at 3 iS:");
					System.out.println(guipos);
					...
					scale = 0.4f;
					guiLocal = new Vec3d(
							i*-0.136f*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale,
							(slim ? 0.135 : 0.125)*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale,
							0.06*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale);
		...
		if ((((IMinecraftVR)mc).getVRSettings().seated || ((IMinecraftVR)mc).getVRSettings().menuAlwaysFollowFace) && ((IEntityRendererVR)mc.entityRenderer).isInMenuRoom()){ //|| mc.vrSettings.vrHudLockMode == VRSettings.HUD_LOCK_BODY) {

			//main menu slow yaw tracking thing
			scale = 2;

			Vec3d posAvg = new Vec3d(0, 0, 0);
			for (Vec3d vec : ((IMinecraftVR)mc).getHmdPosSamples()) {
				posAvg = new Vec3d(posAvg.x + vec.x, posAvg.y + vec.y, posAvg.z + vec.z);
			}
			posAvg = new Vec3d(posAvg.x / ((IMinecraftVR)mc).getHmdPosSamples().size(), posAvg.y / ((IMinecraftVR)mc).getHmdPosSamples().size(), posAvg.z / ((IMinecraftVR)mc).getHmdPosSamples().size());

			float yawAvg = 0;
			for (float f : ((IMinecraftVR)mc).getHmdYawSamples()) {
				yawAvg += f;
			}
			yawAvg /= ((IMinecraftVR)mc).getHmdYawSamples().size();
			yawAvg = (float)Math.toRadians(yawAvg);

			Vec3d dir = new Vec3d(-Math.sin(yawAvg), 0, Math.cos(yawAvg));
			float dist = ((IEntityRendererVR)mc.entityRenderer).isInMenuRoom() ? 2.5F*((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.worldScale: ((IMinecraftVR)mc).getVRSettings().hudDistance;
			...
			guirot = Matrix4f.multiply(gr, Matrix4f.rotationY(((IMinecraftVR)mc).getVRPlayer().vrdata_world_render.rotation_radians));
			guipos = ((IMinecraftVR)mc).getVRPlayer().room_to_world_pos(gpr, ((IMinecraftVR)mc).getVRPlayer().vrdata_world_render);

			System.out.println("GUIPOS at 4 iS:");
			System.out.println(guipos);
		...
		if (currentPass != RenderPass.THIRD) {
			...
			((IEntityRendererVR)mc.entityRenderer).applyMRCameraRotation(false);
		}
		...
		System.out.println("GUIPOS at end iS:");
		System.out.println(guipos);
