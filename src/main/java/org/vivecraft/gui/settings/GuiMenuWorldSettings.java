...
public class GuiMenuWorldSettings extends BaseGuiSettings implements GuiEventEx
{
    ...
    public void initGui()
    {
		...
		this.buttonList.add(new GuiButtonEx(301, this.width / 2 - 155 , 60,150,20, I18n.format("vivecraft.gui.menuworld.refresh")));
		this.buttonList.add(new GuiButtonEx(300, this.width / 2 + 5 , 60 ,150,20, I18n.format("vivecraft.gui.menuworld.loadnew")));
    ...
    protected void actionPerformed(GuiButton par1GuiButton)
    {
        ...
        if (par1GuiButton.enabled)
        {
            ...
            else if (par1GuiButton.id == ID_GENERIC_DEFAULTS)
            {
				...
				try {
					if (((IMinecraftVR)mc).getMenuWorldRenderer().isReady())
                        ((IMinecraftVR)mc).getMenuWorldRenderer().destroy();
                    ((IMinecraftVR)mc).getMenuWorldRenderer().init();
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
			else if (par1GuiButton.id == 301) {
				if (((IMinecraftVR)mc).getMenuWorldRenderer().getWorld() != null) {
					try {
                        ((IMinecraftVR)mc).getMenuWorldRenderer().destroy();
                        ((IMinecraftVR)mc).getMenuWorldRenderer().prepare();
					} catch (Exception e) {
						e.printStackTrace();
					}
				}
