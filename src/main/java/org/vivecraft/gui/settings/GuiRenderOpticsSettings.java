...
public class GuiRenderOpticsSettings  extends BaseGuiSettings implements GuiEventEx
{
    ...
    public void initGui()
    {
...

		{
			...
			for (int i = 0; i < buttons.length; i++) {
				...
				if (option == VRSettings.VrOptions.MIRROR_EYE && ((IMinecraftVR)mc).getVRSettings().displayMirrorMode != VRSettings.MIRROR_ON_CROPPED && ((IMinecraftVR)mc).getVRSettings().displayMirrorMode != VRSettings.MIRROR_ON_SINGLE)
    	...
    	if(((IMinecraftVR)mc).getVRSettings().displayMirrorMode == VRSettings.MIRROR_MIXED_REALITY){
    		...
    		for (int i = 0; i < buttons.length; i++) {
    			VRSettings.VrOptions option = buttons[i];
    			if (option == VRSettings.VrOptions.MONO_FOV && (!((IMinecraftVR)mc).getVRSettings().mixedRealityMRPlusUndistorted || !((IMinecraftVR)mc).getVRSettings().mixedRealityUnityLike))
    				buttons[i] = VRSettings.VrOptions.DUMMY;
    			if (option == VRSettings.VrOptions.MIXED_REALITY_ALPHA_MASK && !((IMinecraftVR)mc).getVRSettings().mixedRealityUnityLike)
    				buttons[i] = VRSettings.VrOptions.DUMMY;
    			if (option == VRSettings.VrOptions.MIXED_REALITY_UNDISTORTED && !((IMinecraftVR)mc).getVRSettings().mixedRealityUnityLike)
    				buttons[i] = VRSettings.VrOptions.DUMMY;
    			if (option == VRSettings.VrOptions.MIXED_REALITY_KEY_COLOR && ((IMinecraftVR)mc).getVRSettings().mixedRealityAlphaMask && ((IMinecraftVR)mc).getVRSettings().mixedRealityUnityLike)
    				buttons[i] = VRSettings.VrOptions.DUMMY;
    		}
    		addButtons(buttons, 75);

    	}else if(((IMinecraftVR)mc).getVRSettings().displayMirrorMode == VRSettings.MIRROR_FIRST_PERSON ){
    ...
    protected void actionPerformed(GuiButton par1GuiButton)
    {
        ...
        IMinecraftVR minecraft = (IMinecraftVR) Minecraft.getMinecraft();
        if (par1GuiButton.enabled)
        {
            if (par1GuiButton.id == ID_GENERIC_DONE)
            {
                minecraft.getVRSettings().saveOptions();
                this.mc.displayGuiScreen(this.parentGuiScreen);
            }
            else if (par1GuiButton.id == ID_GENERIC_DEFAULTS)
            {

                minecraft.getVRSettings().renderScaleFactor = 1.0f;
                minecraft.getVRSettings().displayMirrorMode = VRSettings.MIRROR_ON_CROPPED;
                minecraft.getVRSettings().displayMirrorLeftEye = false;
                minecraft.getVRSettings().mixedRealityKeyColor = new Color();
                minecraft.getVRSettings().mixedRealityRenderHands = false;
                minecraft.getVRSettings().insideBlockSolidColor = false;
                minecraft.getVRSettings().mixedRealityUnityLike = true;
                minecraft.getVRSettings().mixedRealityMRPlusUndistorted = true;
                minecraft.getVRSettings().mixedRealityAlphaMask = false;
                minecraft.getVRSettings().mixedRealityFov = 40;
                ((Minecraft)minecraft).gameSettings.fovSetting = 70f;
                minecraft.getVRSettings().useFsaa = true;
                minecraft.getVRSettings().vrUseStencil = true;
                minecraft.getStereoProvider().reinitFrameBuffers("Load Defaults");
	        ...
	        {
                minecraft.getStereoProvider().reinitFrameBuffers("Setting Change");
	        }
        }
    }
