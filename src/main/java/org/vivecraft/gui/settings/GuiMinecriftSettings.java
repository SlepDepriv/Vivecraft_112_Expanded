...
{    
    ...
    public void initGui()
    {
    	...
    	if(!isConfirm){
    		...
    		GuiSmallButtonEx profilesButton = new GuiSmallButtonEx(mode.getOrdinal(), (this.width / 2 - profileButtonWidth/2 ) , this.height / 4 + 24 , mode._e, I18n.format(mode.getButtonText()));
    		...
    		if(((IMinecraftVR)mc).getVRSettings().seated) {
    			...
    			if(((IMinecraftVR)mc).getVRSettings().allowStandingOriginOffset)
		...
		for (VROption var8 : buttons)
    	{
    		...
    		if(o==null || o.getEnumBoolean() ){
    			...
    			if(o == VrOptions.WORLD_SCALE){
                     ...
                     minValue = 0f;
                     maxValue = 360f;
                     increment = ((IMinecraftVR)mc).getVRSettings().vrWorldRotationIncrement;
    			}
...
//            IHMDInfo hmdInfo = Minecraft.getMinecraft().hmdInfo;
            OpenVRStereoRenderer stereoProvider = ((IMinecraftVR)mc).getStereoProvider();
...
//            IEyePositionProvider positionTracker = Minecraft.getMinecraft().positionTracker;
            ...
            if (par1GuiButton.id < 200 && par1GuiButton instanceof GuiSmallButtonEx)
            {
                ...
                {
                	this.reinit = true;
            		if(((IMinecraftVR)mc).getVRSettings().seated == false){
                    	this.isConfirm = true;
                    	return;
                    }
            ...
            {
                ((IMinecraftVR)mc).getVRSettings().seated = false;
                this.guivrSettings.saveOptions();
            ...
            {
                ((IMinecraftVR)mc).getVRSettings().seated = true;
                this.guivrSettings.saveOptions();
            	this.isConfirm = false;
            	this.reinit = true;
            }
            else if (par1GuiButton.id == ID_GENERIC_DEFAULTS)
            {
                ...
                ((IMinecraftVR)mc).getVRSettings().vrWorldScale = 1;
                ((IMinecraftVR)mc).getVRSettings().vrWorldRotationIncrement = 45f;
                ((IMinecraftVR)mc).getVRSettings().seated = false;
