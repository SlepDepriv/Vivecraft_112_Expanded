...
public class GuiKeyboard extends TwoHandedGuiScreen
{
	...
	public void initGui()
	{
		String arr = ((IMinecraftVR)mc).getVRSettings().keyboardKeys;
		String alt = ((IMinecraftVR)mc).getVRSettings().keyboardKeysShift;
	...
	public static void pressKey(String c) {
		...
		if (mc.currentScreen != null && !((IMinecraftVR)mc).getVRSettings().alwaysSimulateKeyboard) { // experimental, needs testing
			try {
				for (char ch : c.toCharArray()) {
					...
					if (InputInjector.isSupported()) 
						...
						((IGuiScreenVR)mc.currentScreen).keyTypedPublic(ch, code);
					break;
				}
			} catch (Exception e) {
				e.printStackTrace();
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
    	...
    	if(!Display.isActive() && (mc.currentScreen == null || ((IMinecraftVR)mc).getVRSettings().alwaysSimulateKeyboard))
